...
class SimpleEventEmitter:
    """Simple event emitter for streaming pipeline status via SSE"""
    
    def __init__(self, pipeline_id: str, stream: bool = True):
        self.pipeline_id = pipeline_id  
        self.stream = stream
        self.events = []
        self.status_queue = []
    
    async def __call__(self, event_data: dict):
        ...
        self.status_queue.append(event_data)
        logging.info(f"Pipeline Event [{self.pipeline_id}]: {event_data}")
    
    def get_status_events(self):
        """Get and clear queued status events"""
        events = self.status_queue.copy()
...
async def generate_openai_chat_completion(form_data: OpenAIChatCompletionForm):
    ...
    def job():
        ...
        event_emitter = SimpleEventEmitter(pipeline_id, form_data.stream)
        ...
        if form_data.stream:
            ...
            def stream_content():
                ...
                if isinstance(res, str):
                    # Send any queued status events first
                    for status_event in event_emitter.get_status_events():
                        status_message = {
