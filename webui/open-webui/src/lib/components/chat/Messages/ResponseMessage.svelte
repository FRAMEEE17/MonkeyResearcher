<script lang="ts">
	...
	import { config, models, settings, temporaryChatEnabled, TTSWorker, user, showArtifacts, showControls } from '$lib/stores';
	...
	import ResearchPlan from './ResearchPlan.svelte';
	import RealTimeProgress from './RealTimeProgress.svelte';
	...
	// Check if this is a research model
	$: isResearchModel = model && (
		model.id.includes('data_analyst') || 
		model.name?.toLowerCase().includes('deep research') || 
		model.name?.toLowerCase().includes('researcher')
	);
	
	// Check if message has deep research content
	$: hasDeepResearchContent = message?.content && (
		message.content.includes('Deep Research') || 
		message.content.includes('Sources:') || 
		(isResearchModel && message.content.trim().length > 100)
	);

	// Show research plan for first message from research models
	$: showResearchPlan = isResearchModel && message.role === 'assistant' && (
		message.content.trim() === '' || 
		message.statusHistory?.length > 0
	);

	// Show real-time progress for pipeline models
	$: showRealTimeProgress = (isResearchModel || model?.id.includes('pipeline')) && message.role === 'assistant' && (
		message.statusHistory?.length > 0 || message.status
	);
	
	// Auto-open artifacts for completed deep research
	$: if (hasDeepResearchContent && message?.done) {
		setTimeout(() => {
			showControls.set(true);
			showArtifacts.set(true);
		}, 1000); // Delay to ensure smooth animation
	}

	// Get current status for real-time progress
	$: currentStatus = (message?.statusHistory ?? [...(message?.status ? [message?.status] : [])]).at(-1);
		...
		<div class="flex-auto w-0 pl-1 relative -translate-y-0.5">
			...
			<div>
				<div class="chat-{message.role} w-full min-w-full markdown-prose">
					<div>
						<!-- Original status description - commented out to avoid duplication with RealTimeProgress -->
						...
						{#if showResearchPlan}
							<ResearchPlan 
								researchTopic={history.title || 'Research Topic'}
								isResearching={!message?.done}
								progressData={status}
							/>
						{/if}

						{#if showRealTimeProgress}
							<div class="my-3">
								<RealTimeProgress 
									pipelineId={message.id}
									statusData={currentStatus}
									statusHistory={message?.statusHistory ?? []}
									isActive={!message?.done}
								/>
							</div>
						{/if}
						...
						{:else}
							<div class="w-full flex flex-col relative" id="response-content-container">
								{#if message.content === '' && !message.error && (message?.statusHistory ?? [...(message?.status ? [message?.status] : [])]).length === 0}
									<Skeleton />
								{:else if message.content && message.error !== true && !hasDeepResearchContent}
									<!-- always show message contents even if there's an error -->
									<!-- unless message.error === true which is legacy error handling, where the error message is stored in message.content -->
									<!-- Hide deep research content from chat area as it will be shown only in artifacts -->
									<ContentRenderer
										id={message.id}
										{history}
									...
									/>
								{:else if hasDeepResearchContent}
									<!-- Deep Research Content Placeholder -->
									<div class="w-full p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
										<div class="flex items-center gap-3 mb-2">
											<!-- <div class="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center">
												<Sparkles className="w-4 h-4 text-white" />
											</div> -->
											<div>
												<!-- <h3 class="font-semibold text-blue-900 dark:text-blue-100">Deep Research Results</h3> -->
												<p class="text-sm text-blue-700 dark:text-blue-300">Research complete! View the detailed results in the artifact</p>
											</div>
										</div>
										<button 
											class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md transition-all duration-200 hover:scale-105 hover:shadow-lg text-sm font-medium"
											on:click={() => {
												showControls.set(true);
												showArtifacts.set(true);
											}}
