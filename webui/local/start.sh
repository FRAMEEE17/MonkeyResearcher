...
install_frontmatter_requirements() {
  ...
  if [ -n "$requirements" ]; then
    ...
    local uv_command="uv pip install $requirements"
    echo "$uv_command"
...
if [[ "$MODE" == "run" || "$MODE" == "full" ]]; then
  ...
  # Log running processes for deep research pipeline
  # echo "=== PIPELINE PROCESS LOGGING ==="
  # echo "Currently running processes:"
  # ps aux | grep -E "(python|uvicorn|pipeline|search)" | grep -v grep
  # echo "=== END PROCESS LOGGING ==="
  # echo ""
